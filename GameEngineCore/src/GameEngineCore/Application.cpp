...
namespace EliseyAgGameEngine {
	...
	std::array<glm::vec3, 10> positions = {
			...
			glm::vec3( 6.f, -1.f, -4.f),
			glm::vec3( 8.f,  7.f,  5.f),
			glm::vec3( 4.f, -3.f, -8.f),
			glm::vec3(-3.f, -5.f, -2.f),
			glm::vec3(-1.f, -7.f, -8.f),
			glm::vec3( 2.f, -9.f,  1.f)
	};
    ...
    int Application::start(unsigned int window_width, unsigned int window_height, const char* title)
	{
		...
		p_cube_positions_colors_vbo = std::make_unique<VertexBuffer>(pos_norm_uv, sizeof(pos_norm_uv), buffer_layout_vec3_vec3_vec2);
		p_cube_index_buffer = std::make_unique<IndexBuffer>(indices, sizeof(indices) / sizeof(GLuint));

		p_cube_vao->add_vertex_buffer(*p_cube_positions_colors_vbo);
